"""
Credit-card result analysis without run_manifest.csv.
...
1. Final-timestep RMSE tables arranged as epsilon-by-b matrices.
   ...
   The experiment runner performs any carry-forward extension before writing
   the CSV, so this analysis never pads or invents additional rows.
   For every seed,
...
The script discovers running_means_*.csv files directly. It filters by the
...
RESULTS_DIR = "cache/credit_card_b_sweep"
PLOT_DIR = "plots/credit_card/avg_abs_error/vary_method"
...
EPS_VALUES = [1.0, 2.0, 4.0, 8.0, 10.0]
XI_VALUES = [200.0, 1000.0]
B_VALUES = [100, 300, 500, 750, 850, 900, 950, 1000, 2000]
DELTA_VALUES = None
...
def _parse_run_filename(path: Path) -> dict[str, object] | None:
    for pattern in RUN_FILENAME_PATTERNS:
        ...
        if meta.get("clip_upper") is None:
            # The supplied real-data runner uses xi as the clipping upper bound
...
def discover_runs(results_dir: str | Path) -> pd.DataFrame:
    ...
    for path in sorted(root.rglob("running_means_*.csv")):
    ...
    if not rows:
        raise FileNotFoundError(
            f"No parseable running_means_*.csv files were found under {root}."
        )
...
def filter_runs(
    ...
    *,
    ...
    keep = _numeric_mask(runs["n_released"], n)
    filters = {
        "b": b_values,
        "eps": eps_values,
        "xi": xi_values,
...
def _build_final_rmse_lookup(
    *,
    ...
    # Separate cache name because this version pads shorter files by
    # repeating the final observed squared error.
...
def analyze_credit_card_results(
    ...
    *,
    ...
    print(
        ...
        + runs.groupby(["n_released", "eps", "xi", "b", "C_kind"]).size().to_string()
    )
    ...
    if not wrong_length.empty:
        raise ValueError(
            ...
            + wrong_length[
                ["eps", "xi", "b", "C_kind", "seed", "n_released", "file_length", "csv_path"]
            ].to_string(index=False)
        )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Print final-timestep credit-card RMSE tables and generate "
    ...
    parser.add_argument(
        ...
        type=int,
        default=N,
        help="Fixed horizon encoded in every selected experiment CSV.",
    )
    parser.add_argument(
        "--expected-seeds",
        type=int,
